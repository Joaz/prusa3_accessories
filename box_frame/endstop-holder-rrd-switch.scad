...
module endstop(shaft_radius){
    ...
    difference(){
	    union(){
		    ...
		    // for rrd endstops
		    translate([-8, 0, 0]) cube([6, 4, 19]);
		    translate([-28, 0, 0]) cube([6, 4, 19]);
		    translate([-5, 4, 5+13]) rotate([90, 0, 0]) cylinder(h =4, r = 4, $fn = segments);
    	    translate([-(5+screw_hole_spacing), 4, 5+13]) rotate([90, 0, 0]) cylinder(h =4, r = 4, $fn = segments);
